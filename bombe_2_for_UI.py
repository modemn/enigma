...
class Bombe():
    ...
    def __init__(self, t_rotor, m_rotor, b_rotor, indicator, reflector, scrambler_settings, connections, input_letter):
    ...
    def run(self):
            ...
            for closure in closures:
                ...
                for cl in consistent_letters:
                    ...
                    if (self.check_steckers(self.steckers) and valid_stop):
                        ...
                        stop_output = [
                            f'######################## STOP ######################## \n\n Time elapsed: {time:0.04f} seconds \n\n Rotors:, {self.l_rotor}, {self.m_rotor}, {self.r_rotor} \n\n Starting Letters: {self.starting_letters} \n\n Possible Steckers: {self.print_steckers()} \n\n Possible Ring Settings: {self.print_ring_settings()} \n\n ######################## STOP ######################## \n\n {self.crib[0]} <- Plain Crib \n {self.crib[1]} <- Actual Cipher Crib \n {stop_encryption} <- Encrypted Crib With Stop Settings \n\n']

                        self.stops += stop_output
        ...
        end_time = timer.stop()
        self.stops += [f'Total time: {end_time:0.04f} seconds']
        return self.stops
